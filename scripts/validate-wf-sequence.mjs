...
const manifestRecordTotal = s.A4.categories.reduce((n, c) => n + c.expected_records, 0);
const manifestMediaTotal = s.A4.categories.reduce((n, c) => n + c.expected_media_files, 0);
if (manifestRecordTotal !== s.A4.expected_product_record_count) {
  errors.push(`A4 manifest record total mismatch: categories=${manifestRecordTotal}, declared=${s.A4.expected_product_record_count}`);
}
if (manifestMediaTotal !== s.A4.expected_media_file_count) {
  errors.push(`A4 manifest media total mismatch: categories=${manifestMediaTotal}, declared=${s.A4.expected_media_file_count}`);
}
...
if (!fs.existsSync(categoryRoot)) {
  ...
  for (const id of canonical) {
    ...
    if (media.length !== expectedMedia) {
      errors.push(`A4 category ${id}: repository media ${media.length} != Drive-canonical media ${expectedMedia}`);
    }
  }
...
if (mediaTotal !== s.A4.expected_media_file_count) {
  errors.push(`A4 repository media total mismatch: found ${mediaTotal}, Drive-canonical ${s.A4.expected_media_file_count}`);
}
...
console.log(`Canonical DB: ${m.source_of_truth.canonical_product_database_folder}`);
console.log(`A4 records: ${manifestRecordTotal}/${s.A4.expected_product_record_count}`);
